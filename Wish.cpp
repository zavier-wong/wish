...
Wish::Wish(int argc, char **arg) {
    ...
    home_path= getenv("HOME");
    current_work_path=getCwd();
    commands={"alias","bg","cd","echo","exit","fg","history","jobs","kill","pwd","quit","type"};
}
/*
 * shell运行
 * */
...
void Wish::doPipe(std::vector<std::string> &pipeCommand) {
    ...
    for (int i = 1; i < pipeCommand.size(); ++i) {
        if(id==0){
            std::cout<<id<<' '<<getpid()<<' '<<pipeCommand[0]<<std::endl;
            close(p[0][0]);
            dup2(p[0][1],STDOUT_FILENO);
            __do__(pipeCommand[0],pid);
            break;
        }
        if(id==pipeCommand.size()-1){
            std::cout<<id<<' '<<getpid()<<' '<<pipeCommand[i]<<std::endl;
            close(p[i][1]);
            dup2(p[i-1][0],STDIN_FILENO);
            __do__(pipeCommand[i],pid);
            break;
        }
        if(id==i){
            std::cout<<id<<' '<<getpid()<<' '<<pipeCommand[i]<<std::endl;
...
void Wish::__do__(std::string &cmd,pid_t pid){
    int jobState=Jobs::FOREGROUND;
    sigset_t mask;
    std::vector<std::string> arg;
    std::vector<std::pair<int,int>> rd;
    ...
    jobState=Jobs::BACKGROUND;
    jobs.addJob(pid,(jobState==Jobs::FOREGROUND?Jobs::FOREGROUND:Jobs::BACKGROUND), cmd);
    sigprocmask(SIG_UNBLOCK,&mask,NULL);
    //std::cout<<arg.size();
    if (execCommand(arg) < 0) {
        error_msg(arg[0], "Command not found");
...
/*
 * 如果是内建命令就执行，否则返回false；
 * 内建命令：alias，bg，cd，echo，exit，fg，history，jobs，kill，pwd,quit,type
 * */
bool Wish::builtinCommand(const std::vector<std::string>& arg) {
    const std::string &cmd=arg[0];
    if(cmd=="alias"){
...
bool Wish::builtinHelp(const std::vector<std::string> &arg){
    std::cout<<"内建命令：alias，bg，cd，echo，exit，fg，history，jobs，kill，pwd,quit,type\n"
               "外部命令：$PATH 下目录的所有命令\n"
               "退出wish：Ctrl+D, exit , quit"<<std::endl;
    return true;
}
/*
 * 内建命令bg
